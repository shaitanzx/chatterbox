...
# --- Accentuation helpers ---
def convert_plus_to_accent(text: str) -> str:
    """
    Convert ruaccent '+vowel' markers to vowel with combining acute, like VibeVoice.
    """
    replacements = {
        '+а': 'а́', '+А': 'А́',
        '+е': 'е́', '+Е': 'Е́',
...
def get_supported_languages() -> list[str]:
    """
    Return the list of supported languages once multilingual model is loaded.
    """
    return SUPPORTED_LANGUAGES


from engine import load_multilingual_model, get_supported_languages
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not engine.load_model():
            ...
            # 🔄 Automatic loading of the multilingual model at startup
            if config_manager.get_bool("tts_engine.auto_load_multilingual", False):
                try:
                    if load_multilingual_model():
                        logger.info("Multilingual model loaded automatically at startup.")
                    else:
                        logger.warning("Failed to auto-load multilingual model at startup.")
                except Exception as e:
                    logger.error(f"Error auto-loading multilingual model: {e}", exc_info=True)
...
async def restart_engine_endpoint():
    ...
    async def _delayed_restart():
        # Wait a bit (so that the client gets the response) then run the process again
        await asyncio.sleep(2)
        python = sys.executable
        os.execl(python, python, *sys.argv)

    # Start the restart job in the background
...
def convert_plus_to_acute(text: str) -> str:
    # vowel+ → voweĺ
    text = re.sub(rf'([{VOWELS}])\+', r"\1" + ACUTE, text)
    # +vowel → voweĺ
    text = re.sub(rf'\+([{VOWELS}])', r"\1" + ACUTE, text)
    # normalize so that the symbols stick together
...
# --- Loading correction dictionaries (YAML + .dict folder) ---
...
def load_dic_file(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        logger.error(f"Failed to load dic file {path}: {e}", exc_info=True)
    return fixes
...
def load_dict_file(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        logger.error(f"Failed to load dict file {path}: {e}", exc_info=True)
    return fixes
...
def load_custom_accents() -> dict[str, str]:
    ...
    logger.info(f"Loaded {len(combined)} custom accent fixes (YAML + dicts)")
    return combined
...
def build_rules(fixes: dict[str, str]) -> list[tuple[re.Pattern, str]]:
    rules = []
    for k, v in fixes.items():
        if k.startswith("*") and k.endswith("*"):
            # rule contain
            pattern = re.compile(re.escape(k.strip("*")), flags=re.IGNORECASE)
        elif k.startswith("*"):
            # suffix
            pattern = re.compile(re.escape(k.strip("*")) + r"\b", flags=re.IGNORECASE)
        elif k.endswith("*"):
            # prefix
            pattern = re.compile(r"\b" + re.escape(k.strip("*")), flags=re.IGNORECASE)
        else:
            # exact word
            pattern = re.compile(re.escape(k), flags=re.IGNORECASE)
        rules.append((pattern, v))
    return rules

ACCENT_RULES = build_rules(CUSTOM_ACCENTS)

def apply_custom_fixes(text: str) -> str:
    text = unicodedata.normalize("NFC", text)
    # sort: long keys first, short keys second
...
@app.post("/api/accentuate", tags=["Text Processing"])
async def accentuate_text_endpoint(text: str = Body(..., embed=True)):
    if accent_model is None:
        raise HTTPException(status_code=503, detail="RUAccent model not loaded")
    try:
        raw_text = accent_model.process_all(text)  # ruaccent gives +marker
        accented_text = convert_plus_to_accent(raw_text)  # convert to acutes
        accented_text = apply_custom_fixes(accented_text)  # 🔄 use YAML + dict + dic dictionaries
        return {"status": "success", "accented_text": accented_text}
    except Exception as e:
        logger.error(f"Error in accentuate_text_endpoint: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Accentuation failed: {str(e)}")
...
from engine import get_supported_languages
...
async def load_multilingual_model_endpoint():
    """Load the multilingual TTS model and return supported languages."""
    try:
        success = load_multilingual_model()
        if success:
            return {
                "status": "success",
                "message": "Multilingual model loaded successfully",
                "languages": get_supported_languages()
            }
        else:
            raise HTTPException(status_code=500, detail="Failed to load multilingual model")
    except Exception as e:
        logger.error(f"Error in load_multilingual_model_endpoint: {e}", exc_info=True)
...
async def custom_tts_endpoint(
    ...
    try:
        ...
        # --- GLOBAL SPEED CHANGE USING EXTERNAL SOX ---
        speed_factor_to_use = (
        ...
        if speed_factor_to_use != 1.0:
            try:
                ...
                final_audio_tensor = torch.from_numpy(
                    final_audio_np.astype(np.float32)
                )

                from pathlib import Path
                SOX_PATH = str(Path(__file__).parent / "sox" / "sox.exe")

                final_audio_tensor, _ = utils.apply_speed_factor_sox_external(
                    final_audio_tensor,
                ...
                perf_monitor.record(
                    f"Global speed factor {speed_factor_to_use} applied via external sox"
                )

            except Exception as e_sox:
                logger.error(
                    f"Failed to apply global speed via external sox: {e_sox}",
                    exc_info=True,
                )
    ...
    if not encoded_audio_bytes:
        raise HTTPException(
            status_code=500, detail="Failed to encode audio to requested format."
        )
    ...
    # --- Autosave into outputs folder---
    try:
        outputs_dir = get_output_path(ensure_absolute=True)
        outputs_dir.mkdir(parents=True, exist_ok=True)

        # filename: if client gave audio_name, then use it
        base_name = request.audio_name or suggested_filename_base
        file_name = utils.sanitize_filename(f"{base_name}.{output_format_str}")
        file_path = outputs_dir / file_name

        with open(file_path, "wb") as f:
            f.write(encoded_audio_bytes)

        logger.info(f"Autosaving: file saved into {file_path}")
    except Exception as e:
        logger.error(f"Error while saving audiofile: {e}", exc_info=True)
